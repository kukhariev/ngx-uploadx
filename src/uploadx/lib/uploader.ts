...
export abstract class Uploader implements UploadState {
  ...
  private _url = '';
  private useLS = this.options.storeIncompleteUploadUrl !== false;

  get url(): string {
    return this._url || (this.useLS && store.get(this.uploadId)) || '';
  }

  set url(value: string) {
    this._url !== value && this.useLS && store.set(this.uploadId, value);
    this._url = value;
  }
  ...
  private cleanup = () => this.useLS && store.delete(this.uploadId);
